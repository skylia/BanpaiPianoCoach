...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('.notation-note').count(),160);assert.equal(await page.locator('.notation-system').count(),5);
    ...
    assert(scroll.inside>500);assert.equal(scroll.outside,0);assert.equal(await page.locator('.notation-playhead').count(),1);
    ...
    assert.equal(clefCheck.samePitchOffset,125);assert.match(clefCheck.labels,/高音谱表 · 左手/);
