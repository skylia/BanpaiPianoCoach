...
原生 ES Modules 单页应用。Node.js 提供开发静态服务器、可选 DeepSeek 摘要代理及构建复制；采集、曲谱导入、五线谱排版、播放、规则分析和 PDF 导出均在浏览器执行。没有服务端数据库或运行时 npm 安装步骤。VexFlow 4.2.5、Bravura 及 PDF 依赖随源码提供。

托管平台负责私密登录访问。`server.mjs` 不含独立认证，默认只监听 `127.0.0.1:3000`，AI 接口限制本机同源访问。源代码不依赖当前托管平台，可迁到本地或其他具备访问控制的 HTTPS 静态托管服务；纯静态托管不提供 AI 生成接口。
...
| `public/expression.mjs` | 逐音力度有效性、覆盖统计与播放增益映射 |
| `public/playback.mjs` | 示范及回听时间轴，暂停、续播与计时取消 |
| `public/follow.mjs` | 等待弹对的起音组匹配、错误尝试、首次起音节奏及跟弹摘要 |
| `public/follow-ui.mjs` | 跟弹反馈、逐组错误与小节重练入口 |
| `public/coach-document.mjs` | 已保存 AI 报告的文本结构、历史快照与导出元数据校验 |
| `public/coach-pdf.mjs` | 按需加载本机字体和 PDF 引擎，中文换行、A4 分页、依据附页与下载 |
...
| `notes[].velocity` | 可选整数 1–127，来源或录制的 Note On 力度；缺失时不虚构标记 |
...
旧格式 `{pitches:[60,62], lengths:[1,1], bpm:72, title:"..."}` 按顺序累加转换为右手音符。`toLegacyExercise(score)` 仅允许连续右手单音且无末尾空白 / 复杂变速 / 力度信息，避免双手、休止与力度被静默丢弃。普通保存应使用 `serializeScore(score)` 输出 v2。
...
采集设置增加 `options.practiceMode: 'fixed' | 'follow'`，旧记录缺省为固定速度。`FollowSession` 只消费 Note On，以相同起音时刻（误差不超过 1e-6 秒）建立目标组；同组左右手相同音高只要求一次物理落键。当前组使用不超过 120 ms、同时小于相邻目标间距的起音窗口，整个窗口内必须恰好覆盖目标音高。错音、重复键或缺音记一次失败，下一次完整尝试成功后才推进。没有 Note On 时不推进、不累加漏音。窗口使用绝对事件时间，现场匹配和历史重算采用同一个模型。

首次尝试的时间与“上一组成功尝试时间 + 两组原谱起音间隔”比较，第一组相对倒数结束；允许误差按间距的 22% 限制在 80–180 ms。节奏至少 3 个目标组后评分。成功重试与首次尝试之间的时间单独计入纠错等待。跟弹可以比标准曲长更久，最后目标通过后保留所选范围末尾时值；现有后台结束、设备断开、取消倒数、8192 事件上限继续生效，单遍最长 24 小时。

详细 take 保留原始逐音事件、模式、`analysisVersion: 'banpai-follow-1'` 和跟弹摘要。历史反馈按同一算法重算。长期 capture 摘要保留跟弹总组数、已尝试／通过／首次通过组数、错误／缺音／错音尝试数、节奏样本／合拍数与纠错秒数，备份恢复验证计数关系。固定速度聚合与曲目达标排除跟弹指标，练习时长仍累计；AI 摘要按模式分组，跟弹 `firstPass` 不冒充普通 `pitch` 准确率。个人记录仍只在 IndexedDB，不产生 Git 数据文件。
...
导出为类型 1，480 PPQ，含元数据轨、右手轨（通道 1）及左手轨（通道 2）。保留速度变化、拍号、调号、琴键起音、时值、按键力度与曲终；未记录力度的音符导出为 80。休止表现为时间空隙。指法、作品来源、连奏线、表情文字、复杂声部分配不属于本应用 MIDI 导出内容；需无损迁移请用 JSON。

导入从 Note On 保留 `velocity`，量化不改变力度。叠层轨道中相同手别、音高和起拍的重复音，时值与力度分别取最大值，避免结果随轨道顺序变化。`normalizeScore`、JSON 与 `scoreToTimedNotes` 均保留该可选字段。示范通过 `PianoSound.playNote` 将力度映射为 `0.18 * (velocity / 127) ** 0.9` 的增益；无力度或选择均匀对照时使用 0.12，采样内部的三倍增益仍低于 0.55 上限。回听始终使用录下的力度，暂停续播携带同一数值和采样偏移。此映射仅改变音量，不生成自由速度、踏板或 p / f 标记，也不改变评分及用于练习证据关联的音乐指纹。
