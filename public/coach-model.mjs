...
export function buildCoachSummary({activities, learning, takes = [], days = 7, today = dayKey()}) {
  ...
  const summarize = (records, end) => {const s=aggregateActivities(records,{days,today:end});return {minutes:Math.round(s.seconds/60),captureMinutes:Math.round(s.captureSeconds/60),manualMinutes:Math.round(s.manualSeconds/60),practiceDays:s.days,sessions:s.count,midiSessions:records.filter(a=>a.source==='midi').length,virtualSessions:records.filter(a=>a.source==='virtual').length,incompleteSessions:records.filter(a=>a.kind==='capture'&&!a.complete).length,pitch:s.pitch,rhythm:s.rhythm};};
  const groups=new Map();
  for(const a of current.filter(a=>a.kind==='capture')) {
    // Compare like with like: a change of hand, range or tempo is a new group.
    const key=JSON.stringify([a.fingerprint||a.scoreId,a.hand,a.startBar,a.endBar,a.bpm,a.source,a.analysisVersion]);
  ...
  const pieces=[...groups.values()].map(list=>{
    ...
    const metric=a=>({date:a.date,pitch:Math.round(a.correct/(a.expected+a.extras)*100),rhythm:a.paired>=3?Math.round(a.steady/a.paired*100):null});
    return {title:last.title,source:last.source,hand:last.hand,startBar:last.startBar,endBar:last.endBar,bpm:last.bpm,attempts:list.length,complete:complete.length,minutes:Math.round(list.reduce((s,a)=>s+a.seconds,0)/60),first:complete.length?metric(complete[0]):null,last:complete.length?metric(complete.at(-1)):null};
  ...
  for(const t of takes){if(!ids.has('take:'+t.id)||!['midi','virtual'].includes(t.source)||!t.events?.length||t.interrupted)continue;
    try {const rows=analyze(scoreToTimedNotes(t.score,{bpm:t.bpm,...t.options}),t.events,t.bpm).rows; detailed++;
...
export function validateCoachSummary(raw) {
  ...
  const stats=s=>({minutes:num(s.minutes),captureMinutes:num(s.captureMinutes),manualMinutes:num(s.manualMinutes),practiceDays:num(s.practiceDays,30),sessions:num(s.sessions),midiSessions:num(s.midiSessions),virtualSessions:num(s.virtualSessions),incompleteSessions:num(s.incompleteSessions),pitch:metric(s.pitch),rhythm:metric(s.rhythm)});
  const list=(v,max,fn)=>{if(!Array.isArray(v)||v.length>max)throw Error('点评列表无效。');return v.map(fn);};
  const attempt=a=>a===null?null:({date:str(a.date,10),pitch:metric(a.pitch),rhythm:metric(a.rhythm)});
  const hand=v=>{if(!['left','right','both'].includes(v))throw Error('点评声部无效。');return v;};
  return {schemaVersion:1,period:{days:raw.period.days,start:str(raw.period.start,10),end:str(raw.period.end,10)},targetGrade:num(raw.targetGrade,10),weeklyMinutes:num(raw.weeklyMinutes,2520),current:stats(raw.current),previous:stats(raw.previous),pieces:list(raw.pieces,30,p=>({title:str(p.title),source:['midi','virtual'].includes(p.source)?p.source:'unknown',hand:hand(p.hand),startBar:num(p.startBar,2048),endBar:num(p.endBar,2048),bpm:num(p.bpm,1000),attempts:num(p.attempts),complete:num(p.complete),minutes:num(p.minutes),first:attempt(p.first),last:attempt(p.last)})),hotspots:list(raw.hotspots,12,h=>({title:str(h.title),bar:num(h.bar,2048),hand:hand(h.hand),issues:num(h.issues)})),evidence:{detailedCompleteTakes:num(raw.evidence.detailedCompleteTakes,100),captureSessions:num(raw.evidence.captureSessions),invalidRecords:num(raw.evidence.invalidRecords),limitations:list(raw.evidence.limitations,8,s=>str(s,300))},plans:list(raw.plans,20,p=>({title:str(p.title),grade:num(p.grade,10),targetBpm:num(p.targetBpm,1000),hand:hand(p.hand),status:str(p.status,20)})),checklist:{done:num(raw.checklist.done,120),review:num(raw.checklist.review,120)}};
}
