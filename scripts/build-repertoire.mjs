...
import {dynamicsInfo} from '../public/expression.mjs';
...
const defs={
 ...
 3:[...[2,6,7,8,9,10,14,16].map(sch),'25EF-01','25EF-05',...[1,2,3].map(n=>'clementi-op36-no1-movement'+n)],
...
for(const [grade,names] of Object.entries(defs))for(const name of names){
 ...
 if(meta.sourceSha256&&createHash('sha256').update(sourceText).digest('hex')!==meta.sourceSha256)throw Error(name+' notation source checksum mismatch');
 ...
 let title,composer,category='piece',collection,versionNote='';
 ...
 else if(name.startsWith('clementi-op36-no1-movement')){const n=meta.movement;title=`C 大调小奏鸣曲 · Op.36 No.1 · 第${['一','二','三'][n-1]}乐章（${['Spiritoso','Andante','Vivace'][n-1]}）`;composer='克列门蒂';collection='克列门蒂小奏鸣曲 Op.36 No.1';versionNote=`第 ${n} 乐章全段，三个乐章分别收录；${n===1?'第一乐章反复段各弹一遍，不展开反复。':n===2?'本乐章转为 F 大调，保留三连音及源 MIDI 的装饰音时值。':'本乐章为 3/8 拍，源谱八分音符速度 160 对应本应用四分音符 BPM 80。'}`;}
 ...
 const s=normalizeScore({...score,id,title,composer,collection,grade:Number(grade),category,grading:'general',level:`通用参考 ${grade} 级`,source:{label:`Mutopia · ${maintainer}`,url:page,note:`${license}（本条数据沿用该许可）；底本：${value('source')||'见来源页'}。${versionNote}收录所列 MIDI 的全部音符，按原上下谱表轨道分左右手，不按音高自动分手；反复及装饰以 MIDI 实际包含的内容为准，非原版完整记谱。保留原始音高、起音、时值、按键力度及变速；谱面重新排版。${parsed.warnings.join('')}分级为半拍通用参考，未核实为央音第四套指定条目。`},tags:[grade+'级',category==='study'?'练习曲':category==='polyphony'?'复调':'乐曲',meta.movement?'完整乐章':'MIDI 全曲']});
 rows.push({score:s,source:{...meta,license,maintainer,page,grade:Number(grade),title,notes:s.notes.length,bars:barCount(s),dynamics:dynamicsInfo(s.notes),handTracks:parsed.tracks.map(t=>({id:t.id,name:t.name})),warnings:parsed.warnings}});
}
...
const graded=REPERTOIRE.filter(s=>s.grade),counts=Array.from({length:10},(_,i)=>graded.filter(s=>s.grade===i+1).length);
const repertoireLines=['# 1–10 级分级曲库','', `每级至少 10 个练习条目，共 ${graded.length} 个。三级保留原有 10 首，另增克列门蒂 Op.36 No.1 三个完整乐章，分别列出以便独立跟练。各级条目数：${counts.join(' / ')}。等级全部为通用参考难度，不是央音指定教材清单。1 级为公版旋律的入门编配，主题／副歌节选已标明；2–10 级完整保留所列 MIDI 的全部音符，反复和装饰以源 MIDI 为准，软件重新排版。原有 9 个基础条目另存旧版区，不计入本表。`,'', '| 参考等级 | 曲目 | 作曲者 | 收录版本 |','| --- | --- | --- | --- |'];
